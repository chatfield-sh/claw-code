...
-- ---------------------------------------------------------------------------
-- Brief snapshots (generated by the nightly cron; the daily loop's landing pad)
-- ---------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS brief_snapshot (
    id          uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    tenant_id   uuid NOT NULL REFERENCES tenant(id),
    user_id     uuid NOT NULL REFERENCES user_profile(id),
    eod         boolean NOT NULL DEFAULT false,  -- false = morning brief, true = EOD recap
    headline    text,
    payload     jsonb NOT NULL DEFAULT '{}',     -- the full BriefResponse
    created_at  timestamptz NOT NULL DEFAULT now()
);
CREATE INDEX IF NOT EXISTS idx_brief_tenant ON brief_snapshot(tenant_id, created_at DESC);
